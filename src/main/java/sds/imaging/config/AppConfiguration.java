...
public class AppConfiguration {
    ...
    @Bean
    BlockingQueue<GenerateImage> getProcessingQueue(
            @Value("${QUEUE_PREFETCH_SIZE:15}") Integer prefetchSize) {
        LOGGER.debug("QUEUE_PREFETCH_SIZE is set to {}", prefetchSize);
        return new ArrayBlockingQueue<GenerateImage>(prefetchSize);
    }    
    
    @Bean
    BlockingQueue<GenerateImageJmol> getJmolProcessingQueue(
            @Value("${QUEUE_PREFETCH_SIZE:15}") Integer prefetchSize) {
        LOGGER.debug("QUEUE_PREFETCH_SIZE is set to {}", prefetchSize);
        return new ArrayBlockingQueue<GenerateImageJmol>(prefetchSize);
    }
}
